...
data/
  ...
  ruo-{region}/       raw RUO files for the 7th-grade min/max scores pages, one dir per
                      ...
                      2026 klasirane 1 only, imported for ALL 28 regions. Most go through
                      tmp-ruo-2026/build.py; Haskovo/Pernik use their own scripts and Vratsa
                      2026 uses tmp-ruo-vraca/build_2026.py (its file dropped the settlement
                      + school-code columns). 2026 profile codes are 5-digit everywhere,
                      zero-padded in some regions — RuoDecomplexor canonicalizes them.
                      Gabrovo 2026 arrived as an image-only scanned PDF; the user hand-typed
                      a machine-readable XLSX from it (standard 21-col layout) which is what
                      is now imported. Montana 2026 has one source row with min>max dropped
                      as impossible)

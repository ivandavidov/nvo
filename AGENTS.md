...
- Data source: `data.egov.bg` for the main site plus RUO Sofia XLSX files for the beta `klasirane` pages

Main public pages:
- `docs/4/` — НВО след 4 клас
- `docs/7/` — НВО след 7 клас
- `docs/7/klasirane/` — Beta pages for 7th-grade admissions rankings by class/profile (currently Sofia)
...
./all.sh
```
Builds Java tool, normalizes CSV/XLSX files, regenerates all `schools-{4,7,10,12}.js` files, generates the API (`docs/api/v1/`), city/year landing pages (`docs/{grade}/{city|year}/`), beta `docs/7/klasirane/` pages, and `docs/sitemap.xml`.
...
```
- scans all generated landing pages, static pages, and nested beta pages under `docs/*/klasirane/`
- writes `docs/sitemap.xml`

### Beta RUO Sofia pipeline
```bash
...
- `RuoDecomplexor` converts the normalized RUO CSV files into `docs/js/ruo-sofia.js`
- `RuoPage` generates `docs/7/klasirane/sofia/index.html`
- This feature is publishable but still considered beta; preserve existing behavior and avoid broad refactors unless requested
...
data/
  ...
  ruo-sofia/          raw XLSX files for the beta 7th-grade admissions rankings page
  ...
  normalized/         processed CSV files (generated by NvoDziNormalizer)
                      ...
                      beta RUO naming: ruo-sofia-{2023-2025}-normalized.csv
...
data/ruo-sofia/*.xlsx -> data/normalized/ruo-sofia-*.csv -> java (RuoDecomplexor)
                                                          ...
                                                          -> java (RuoPage)
                                                          -> docs/7/klasirane/sofia/index.html
```
...
docs/
  ...
  7/klasirane/{city}/        beta ranking pages by city for 7th-grade admissions
  ...
  js/
    ...
    ruo-sofia.js               generated beta admissions-ranking data for Sofia (DO NOT EDIT)
...
java/src/main/java/nvo/
  ...
  RuoNormalizer.java      XLSX → normalized CSV for beta RUO Sofia admissions rankings
  RuoDecomplexor.java     normalized RUO CSV → docs/js/ruo-{city}.js
  RuoPage.java            generated beta HTML page under docs/7/klasirane/{city}/
...
1. Do not manually edit generated files: `docs/js/schools-{4,7,10,12}.js`, `docs/js/ruo-*.js`, `docs/api/v1/` (all contents), and generated `docs/7/klasirane/**/index.html` pages.
...
6. Beta `klasirane` pages are generated standalone pages with inline JS/CSS; they do not use the shared `logic-*.js` split architecture.
...
- beta `docs/7/klasirane/sofia/` loads its table, filters, charts, search/sort, and relative asset paths correctly
...
- `docs/js/ruo-sofia.js` — generated beta data for the Sofia admissions rankings page
...
- `docs/7/klasirane/sofia/index.html` — generated beta page for 7th-grade admissions rankings in Sofia
- Java source files under `java/src/main/java/nvo/`:
  ...
  - `RuoNormalizer.java` — XLSX normalization for the beta RUO Sofia flow
  - `RuoDecomplexor.java` — normalized RUO CSV → generated `ruo-{city}.js`
  - `RuoPage.java` — generated HTML page for `docs/7/klasirane/{city}/`
